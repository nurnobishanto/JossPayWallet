...
@section('title', __('global.stores'))

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>{{__('global.stores')}}</h1>
            @can('store_create')
                <a href="{{route('admin.stores.create')}}" class="btn btn-primary mt-2">{{__('global.add_new')}}</a>
            @endcan
            @can('store_delete')
                <a href="{{route('admin.stores.trashed')}}" class="btn btn-danger mt-2">{{__('global.trash_list')}}</a>
            @endcan
        ...
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">{{__('global.stores')}}</li>
            </ol>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <form action="{{ route('admin.stores.destroy', $store->id) }}" method="POST">
                                            ...
                                            @csrf
                                            @can('store_view')
                                                <a href="{{route('admin.stores.show',['store'=>$store->id])}}" class="btn btn-info px-1 py-0 btn-sm"><i class="fa fa-eye"></i></a>
                                            @endcan
                                            @can('store_update')
                                                <a href="{{route('admin.stores.edit',['store'=>$store->id])}}" class="btn btn-warning px-1 py-0 btn-sm"><i class="fa fa-pen"></i></a>
                                            @endcan
                                            @can('store_delete')
