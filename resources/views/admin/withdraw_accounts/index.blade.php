...
@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1>{{__('global.withdraw_accounts')}}</h1>
            @can('withdraw_account_create')
                <a href="{{route('admin.withdraw-accounts.create')}}" class="btn btn-primary mt-2">{{__('global.add_new')}}</a>
            @endcan
            @can('withdraw_account_delete')
                <a href="{{route('admin.withdraw-accounts.trashed')}}" class="btn btn-danger mt-2">{{__('global.trash_list')}}</a>
            @endcan
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <form action="{{ route('admin.withdraw-accounts.destroy', $withdraw_account->id) }}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                            @can('withdraw_account_view')
                                                <a href="{{route('admin.withdraw-accounts.show',['withdraw_account'=>$withdraw_account->id])}}" class="btn btn-info px-1 py-0 btn-sm"><i class="fa fa-eye"></i></a>
                                            @endcan
                                            @can('withdraw_account_update')
                                                <a href="{{route('admin.withdraw-accounts.edit',['withdraw_account'=>$withdraw_account->id])}}" class="btn btn-warning px-1 py-0 btn-sm"><i class="fa fa-pen"></i></a>
                                            @endcan
                                            @can('withdraw_account_delete')
